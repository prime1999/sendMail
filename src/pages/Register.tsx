		...
		try {
			...
			if (!passwordRegex.test(password)) {
				setErrorData((prev: any) => ({
					...prev,
					passwordError:
						"Password length should be less than 15 & greater than 8",
				}));
			}
						...
						<form className="w-4/5 md:w-2/5 lg:w-1/5">
							...
							<div>
								...
								<button
									...
									className="w-full h-8 button-gradient p-2 rounded-md mt-4 font-inter font-medium text-xs"
								>
									{createUser.isPending ? <Spinner /> : "Create Account"}
								</button>
							</div>
							<div className="w-full text-xs font-inter font-medium flex text-gray-500 items-center justify-center mt-2">
								<Link to="/signIn" className="duration-500 hover:text-gray-800">
									Sign-in instead
								</Link>
							</div>
						</form>
